...
*                        -a lat         decimal latitude (default = 40.1167)
*                        -o lon         decimal longitude (default = 88.2167) 
*                        -z tz          timezone (default = 6, cst)
...
int day;
int yr;
int tz=6;                       /* Default time zone */
char *tzs = "(CST)";            /* Default time zone string */
char *dtzs = "(CDT)";           /* Default daylight savings time string */
int popt = 0;

double lat = 40.1167;           /* Default latitude (Champaign, IL) */
double lon = 88.2167;           /* Default Longitude (Degrees west) */ 
...
char *argv[];
{
    ...
    if (popt) {
        ...
        dh_to_hm(alt, &h, &m);
        printf("The sun is at altitude: %2d deg %02d min", h, m);
        dh_to_hm(az, &h, &m);
        printf("   azimuth:  %2d deg %02d min. \n", h, m);
    }
}
...
julian_date(int m, int d, int y)
{
    ...
    a = y/100;
    b = 2 - a + a/4;
    b += (longword)((double)y * 365.25);
    b += (longword)(30.6001 * ((double)m + 1.0));
    jd = (double)d + (double)b + 1720994.5;
    return(jd);
}

double 
hms_to_dh(int h, int m, int s)
{
    double rv;
