...
void initgetrec(void)
{
	...
	uchar *p;
        ...
        dprintf( ("inside initgetrec %06lX", lookup("FILENAME",symtab)) );
	for (i = 1; i < *ARGC; i++) {
...
int getrec(uchar *buf)	/* get next input record from whatever source */
...
int readrec(uchar *buf, int bufsize, FILE *inf)	/* read one record into buf */
{
	register int sep, c;
	register uchar *rr;
	register int nrr;
...
int refldbld(uchar *rec, uchar *fs)	/* build fields from reg expr in FS */
{
	uchar *fr;
	int i, tempstat;
	fa *pfa;
...
int isclvar(uchar *s)	/* is s of form var=something ? */
{
	uchar *os = s;
...
int isnumber(uchar *s)	/* probably should be done by a library function */
{
	register int d1, d2;
	int point;
	uchar *es;
