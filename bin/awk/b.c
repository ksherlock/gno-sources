...
int member(int c, uchar *s)	/* is c in s? */
{
	while (*s)
		if (c == *s++)
			return(1);
...
int pmatch(fa *f, uchar *p)	/* longest match, for sub */
{
	register int s, ns;
	register uchar *q;
	int i, k;
...
int nematch(fa *f, uchar *p)	/* non-empty match, for sub */
{
	register int s, ns;
	register uchar *q;
	int i, k;
