...
GSString255Ptr n;
    ...
    if (f.fileType == 0x0F) { /* directory? */
    	fprintf(stderr,"chtyp: %s is a directory\n",filename);
        free(n); return; /* skip this entry, but don't abort */
    }
    if (t != -1) f.fileType = t;
    if (a != -1) {
	    f.auxType = (unsigned long) a;
	    f.auxType &= 0xFFFF;
    }	    
    SetFileInfoGS(&f);
    if (_toolErr) {
	...
	switch (ch) {
	    case 'l':
            ...
            case 'a':
            	if (aopt != -1) usage();
	            if (isdigit(optarg[0]))
	                sscanf(optarg,"%u",&aopt);
                else if (optarg[0] == '$')
	                sscanf(optarg+1,"%x",&aopt);
		else {
                    fprintf(stderr,"chtyp: invalid option argument '%s'\n",optarg);
                    exit(1);
                }
                break;
            case 'v':
