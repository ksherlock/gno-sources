...
int 	getnumeric (char *p);
void 	read_if (void);
int 	getval (char *p, char *p_argtyp);
int 	do_oper (int first, int oper, int second);
void 	set (int *param, int val, char type, int defval, int minval, int maxval);
int 	set_ireg (char *name, int val, int opt);
int	expesc ();
int	specialchar ();
int	fontchange ();
...
int 	colstr (char *p, char *d);
int 	putstr (char *name, char *p);
