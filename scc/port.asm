...
* Line Discipline entry points
	...
	ds	t_signalIO-editInd
               dc	i4'nullfunc'	; signalIO space
	dc	i2'$FFFF'
	dc	i4'D_select'
	ds	ttyRecSize-t_selwakeup
	END
D_in_enq	START
	rtl
	END
...
* record that the process wants to do I/O
	...
	jsl	>dtty+t_GetProcInd
	sta	procPtr
	stx	procPtr+2

               ldy	#P_waitvec
	lda	[procPtr],y	; is it this process?
	cmp	>dtty+t_selwakeup	; #selwait
	bne	nocollision
	ldy	#P_waitvec+2
	lda	[procPtr],y
	cmp	>dtty+t_selwakeup+2 ;#^selwait
	bne	nocollision	; sleeping on something else now
...
nocollision    jsl	>dtty+t_GetProcInd		; set select_proc field to
               tya
	sta	>dtty+t_select_proc	; current process ID
	bra	doexcept	; no data, return 0
...
doexcept	lda	#0	; there are no exceptions on console
	sta	res
